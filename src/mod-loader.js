(function () {
  ...
  async function loadManifest(manifestUrl = '../mods/mods.json', options = {}) {
    const { requiredVersion } = options;
    ...
    const minecraftVersion = manifest.minecraftVersion ?? 'unspecified';
    if (requiredVersion && minecraftVersion !== requiredVersion) {
      throw new Error(
        `Expected Minecraft version ${requiredVersion} but manifest is ${minecraftVersion}`
      );
    }
